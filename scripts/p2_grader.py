...
class aws_grader:
    def __init__(
        self, access_keyId, access_key, req_sqs, resp_sqs, in_bucket, out_bucket
    ):
    ...
    def validate_sqs_queues(self):
        ...
        try:
            ...
            resp_sqs = self.sqs_resources.get_queue_by_name(
                QueueName=self.resp_sqs_name
            )
            ...
            print(
                f"SQS Request Queue:{self.req_sqs_name} has {ip_queue_requests} pending messages."
            )
            print(
                f"SQS Response Queue:{self.resp_sqs_name} has {op_queue_response} pending messages."
            )
    ...
    def beautify_headers(self):
        ...
        for line1, line2, line3, line4, line5 in zip(
            ...
            print(
                f"| {line1.center(column_width)} | {line2.center(column_width)} | {line3.center(column_width)} | {line4.center(column_width)} | {line5.center(column_width)} |"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--access_keyId", type=str, help="ACCCESS KEY ID of the grading IAM user"
    )
    parser.add_argument(
        "--access_key", type=str, help="SECRET ACCCESS KEY of the grading IAM user"
    )
    ...
    aws_obj = aws_grader(
        access_keyId, access_key, req_sqs, resp_sqs, in_bucket, out_bucket
    )
    aws_obj.main()
